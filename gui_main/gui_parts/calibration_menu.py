...
def create_calibration_menu(state: State, font, aspect_ratio):
    ...
    ### TOGGLE ###
    calibration_enable_toggle = Toggle(
        position = [0.775, 0.24],
        offset = [0.0, 0.015*aspect_ratio],
        id = "calibration_enable_toggle",
        scale = [0.04, 0.04],
        initial_state = 1,
        aspect_ratio = aspect_ratio,
        on_click = lambda button, gui, custom_data: calibration_toggle_on_click(button, gui, custom_data)
    )

    def calibration_toggle_on_click(button, gui, state):
        if button.mouse_click_count % 2 == 0:
            state.echolib_handler.append_camera_command("CalibrationOff")
        else:
            state.echolib_handler.append_camera_command("CalibrationOn")

    calibration_enable_toggle.depends_on(element = button_container_2)
    ...
    return [d_original, d_zoom, calibration_title, advanced_mode, advanced_mode_toggle, button_container, button_container_2, calibration_enable_toggle]
