...
class ImageCorrection:
    def __init__(self, reference_paths, illumination_settings, color_settings, illum_map=None, ccm=None):
        ...
        self.illumination_map = illum_map
        self.ccm = ccm
        

        
    def load_ref_images(self, reference_paths):
        ref_white = cv2.imread(reference_paths['white'])
        self.ref_white = cv2.cvtColor(ref_white, cv2.COLOR_BGR2RGB)
        ref_cal = cv2.imread(reference_paths['cal'])
        self.ref_cal = cv2.cvtColor(ref_cal, cv2.COLOR_BGR2RGB)
        
        if any(img is None for img in [self.ref_white, self.ref_cal]):
    ...
    #     def gamma_decode_srgb(img):
    #         return np.power(img, 2.2)

    #     def gamma_encode_srgb(img):
    ...
    #     linear = gamma_decode_srgb(img)

    #     corrected = np.empty_like(linear)
    #     for c in range(3):
    #         corrected[:, :, c] = linear[:, :, c] * self.illumination_map

    #     h, w, _ = corrected.shape
    #     reshaped = corrected.reshape(-1, 3)
    #     corrected = reshaped @ self.ccm.T
    #     corrected = np.clip(corrected, 0, 1).reshape(h, w, 3)

    #     corrected = gamma_encode_srgb(corrected)
    ...
    def calibrate(self, target_cal, target_white):
        ...
        np.save("./data/reference_condition/ccm.npy", self.ccm.astype(np.float32))
        np.save("./data/reference_condition/illum_map.npy", self.illumination_map.astype(np.float32))
